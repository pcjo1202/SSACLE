...
public class UserController implements UserSwaggerController{
    ...
    @Override
    public ResponseEntity<Void> createStudent(@RequestBody @Valid JoinDTO joinDTO, String verificationCode) {
        joinService.joinStudent(joinDTO,verificationCode);
        return ResponseEntity.status(HttpStatus.CREATED).build();
    }

    @Override
    public ResponseEntity<Void> createAlumni(JoinDTO joinDTO, String verificationCode) {
        joinService.joinAlumni(joinDTO,verificationCode);
        return ResponseEntity.status(HttpStatus.CREATED).build();
    }

    @Override
    public ResponseEntity<Void> createAdmin(JoinDTO joinDTO, String verificationCode) {
        joinService.joinAdmin(joinDTO, verificationCode);
        return ResponseEntity.status(HttpStatus.CREATED).build();
    }
